...
2026-09-12 当前[箭头与菜单修复专项](controls-readability.md#验证与交付)：本机Chrome 153.0.8010.36 / headless / 标准画质 / 1920×1080 / DPR1，三档正常47 / 57 / 72 Combo各约42.23秒有效采样，帧间隔p95 16.7 / 16.7 / 16.7ms；按键至可见反馈p95 26.5 / 27.1 / 27.3ms，全部≤33.3ms。原始帧数组、资源与退出回收见[当前摘要](historical-evidence-files.md#file-db578d29f9a2cef4)。这是同源码acceptance包的防火墙专项，没有重跑原V05的60秒密集机关和30次区域往返。

2026-09-12 前次[防火墙v3专项](firewall-hazards.md#验证与交付)：本机 Chrome 153.0.8010.36 / headless / 标准画质 / 1920×1080 / DPR1，正式47/57/72 Combo各约42.23秒有效采样，帧间隔中位均约16.7ms、p95为16.7 / 16.7 / 16.7ms；按键至可见反馈p95为26.6 / 27.9 / 28.0ms，所有样本≤33.8ms。具体资源计数、退出释放及原始逐帧记录见[性能摘要](historical-evidence-files.md#file-ad0360a56e44ed37)。测量使用同源码acceptance包；不替代原M5的60秒与30次往返实测。

2026-09-12 前次[防火墙 v2 专项](firewall-restoration.md#4-验证与交付)：本机 Chrome 153 / headless / 标准画质 / 1920×1080 / DPR1，三档正常游玩各约 42.23 秒有效采样。帧间隔中位数均约16.7ms、p95最高16.8ms；按键至可见命中反馈p95最高31.1ms、所有样本≤32.4ms。活动态58 draw calls /18几何/9纹理，退出侧屏释放。原始帧数组、方法与条件见[性能摘要](historical-evidence-files.md#file-af828eecf4483b9f)及关联浏览器记录；不替代下面原M5的60秒/30次往返实测。
...
原证据：[标准画质](historical-evidence-files.md#file-660f3ceb28625128)、[低画质](historical-evidence-files.md#file-d8819432d4a00ad1)。
...
[原始60条落点和资源记录](historical-evidence-files.md#file-2055ab54d0b7d8df)通过正常键盘 M 打开区域图，再点击可见的已激活传送按钮。按 A、B、C、D、仓库循环六轮，每次到达目的区域后返回中心，共30次往返、60条正式落点。
...
原证据：[标准画质](historical-evidence-files.md#file-50b8171b97df4967)、[低画质](historical-evidence-files.md#file-7c8b09bc7ffbd8f3)。本机原生Safari当日安装版本为26.6.2（21624.5.1.11.3），仍属M1 Pro / 32 GB补充环境。版本身份与完整发布核对边界见[浏览器矩阵](browser-matrix.md)。
...
| 证据                                                                         | 实际结果                                                                                   | 不作通过的原因                                                                                 |
| ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------- |
| [中心教学快照](historical-evidence-files.md#file-634195c5536e729d)           | `firstGamePreparationMs=28405`                                                             | 首次准备异常原因尚未由本组证据解释；后续较快继续不能删除它                                     |
| [十秒诊断](historical-evidence-files.md#file-cea0450b7fa4611f)               | 宿主11375ms，599间隔，中位17ms/p9518ms，输入0；末态manual暂停                              | 中心场景、窗口不足60秒，且为暂停诊断                                                           |
| [后次缓存继续](historical-evidence-files.md#file-01074ad8f6354ec8)           | 宿主5356ms，187间隔，中位17ms/p9525ms，输入0；第一次场景准备60ms                           | 约3秒短时采样，末态manual暂停，不是最密场景持续60秒                                            |
| [音频焦点修复快照](historical-evidence-files.md#file-aebf4e285d680597)       | 单个输入19ms；启动入口27059ms、第一次场景准备48ms                                          | 故障恢复功能证据，单输入和暂停末态不能填成性能通过                                             |
| [最密场景首轮未聚焦诊断](historical-evidence-files.md#file-b5e12a98f0567b76) | 帧汇总6490间隔、113.163s，中位17ms/p9523ms；宿主99.167s；输入5→5、generation77→77、撤销0→0 | 计划的60方向没有成为游戏步骤，不能当作已接受输入或普通移动性能；后续正式两档不覆盖这一失败事实 |

### 5.3 Safari正常缓存继续与会话诊断

[正常缓存继续](historical-evidence-files.md#file-6ba3c35af919ef7e)记录原生任务窗口前置、普通URL、实际“继续”后正常Right：宿主操作上界1195ms，覆盖原生AX、键盘和只读回读开销；第一次游戏准备87ms。正常结果位于 `a.t.2.4`，Right实际领取5单位，6/130、generation61，audio=true、无暂停。这支持**该次缓存继续到可操作的≤3秒子项**，不替代所有profile、设备或冷加载。
...
[同源会话导航诊断](historical-evidence-files.md#file-6c21a2d8a2866119)中，原生地址栏进入 `?session-check=1` 后，1914ms宿主观测仍显示busy、没有继续按钮；再次导航到 `/` 后，1479ms观测显示继续且不busy。记录未点击“重试获取会话”；内部锁释放时序未被观测，只有后一条导航恢复事实，**不能据此宣称会话缺陷已修复或全部导航路径通过**。

### 5.4 普通移动动画的实际渲染完成时间

以下先保留**原120ms视觉目标**的实测。[Codex内嵌浏览器六步记录](historical-evidence-files.md#file-59fafed88ab349d8)以正常键盘Left/Right单步，每步等待220ms后只读取得 `render.lastPlayerMovement`。环境为本机M1 Pro、M5/content4、1366×768/DPR1、标准画质、缩放1、未减少动态；浏览器完整引擎版本未采集，不并入独立Chrome列。起终均无暂停，六步在 `a.t.1.-2` 与 `a.t.0.-2` 往返，generation152→158，26单位与永久进度不变。
...
[此前DOM轮询](historical-evidence-files.md#file-d07c964106c0e119)的宿主上界145–154ms含输入工具、DOM回读及一帧滞后，不据此判实际动画失败；[误用旧目标布局的诊断](historical-evidence-files.md#file-07a2c6d3775a96d3)没有等到新目标，不能作通过证据。Safari两档末次移动分别135/129ms只是各一条观察，标准热身仍实测159ms超过140ms；保留该超限，不能用两个末次值声明Safari普通移动全量通过。
...
| ---------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | --------------------------------------------- | ----------------------------------------------------- |
| [IAB世界图](historical-evidence-files.md#file-f420c7e2a2410efb)，1366×768、DPR1、标准、缩放1、未减少动态；M5/content4，26单位                        | A的 `1,-2 ↔ 0,-2`，Left/Right三组；generation158→164，逐步159–164；190ms观察等待              | 107.8 / 107.6 / 108.8 / 109.5 / 104.8 / 108.6 | **104.8–109.5ms；本组六步≤140ms通过**                 |
| [Safari64格](historical-evidence-files.md#file-b29f6caf2707f94f)，1455×854、浏览器/渲染DPR2、标准、缩放1、未减少动态；M5/content4，130单位性能设置档 | `c.theft.03.t.1.6 ↔ 1.5`，Up/Down三组；generation225→231，逐步226–231，撤销0→6；210ms观察等待 | 113 / 116 / 110 / 114 / 101 / 105             | **101–116ms；本组六步≤140ms通过，含新建场景首次热身** |
...
Safari通过正常文件选择和确认导入真实M4全收集档，仅作性能设置来源，再以区域图M选择C、Up×2、Left×6、F/F正常进入独立练习；没有修改位置或完成状态，也没有把导入130档当作Safari通关。结束后通过正常UI导入恢复原有6单位备份，[恢复记录](historical-evidence-files.md#file-15ba79740a72cfb4)为generation232、`a.t.1.4`、6/130、2领取，标准设置、未减少动态、音频启用、无暂停；没有把临时全收集性能档留作该浏览器进度。
...
| 已缓存资源、正常profile，从继续到可操作≤3秒 | Chrome第一次场景准备51.4 / 105.6ms；[正式包正常继续与刷新](historical-evidence-files.md#file-53ae96ea9721c781)有功能记录 | 场景准备是部分阶段，正式包操作记录没有这一端到端计时；不能相加旧阶段或采用Safari1195ms代填         |
...
| --------------------------- | ---------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| 点击“继续游戏”到画布可操作  | canvas可见且`aria-busy=false`、过场隐藏、对话框关闭：宿主320ms；随后真实ArrowLeft与已保存状态：390ms | [计时与实际导出核对](historical-evidence-files.md#file-1c1bd827a4b29eae)：947代仓库7,0→948代6,0；26奖励、目标和9完成布局保持                                                  | 本机本次≤3秒通过           |
| 正常Right回终端、刷新后继续 | 再次点击继续到画布就绪：宿主315ms                                                                    | [刷新恢复](historical-evidence-files.md#file-dc71f2d0f8736fc1)：949代仓库7,0；[实际UI导出](historical-evidence-files.md#file-9c086cb21cf87352)的payload与刷新前逐字段完全相同 | 本机本次≤3秒且刷新恢复通过 |
...
先在仓库正常Left/Right三组，再通过区域图C、Up两步、Left六步、F进入完成房、F进入独立练习，正常Up/Down三组；[进入记录](historical-evidence-files.md#file-6886ddedc29d03aa)保留逐步落点。没有导入性能初态、重设位置或改动地图。

| 场景                                                                    | 六次实际完成时间（ms，表内保留1位小数）       | 检查结果                                                                  |
| ----------------------------------------------------------------------- | --------------------------------------------- | ------------------------------------------------------------------------- |
| [仓库世界](historical-evidence-files.md#file-9a008accf446b868)：7,0↔6,0 | 108.3 / 107.5 / 108.7 / 104.7 / 110.2 / 110.4 | 6步≤140ms；每步相机实际focus为对应6,0或7,0，玩家与镜头都已到位，951→957代 |
| [64格房间](historical-evidence-files.md#file-914c5fb53460dba0)：1,6↔1,5 | 105.7 / 104.4 / 107.5 / 110.9 / 105.4 / 106.3 | 6步≤140ms；局部整盘镜头保持固定，968→974代、撤销深度0→6                   |
...
| 指标                     | [标准画质](historical-evidence-files.md#file-43d30155e760becc) | [低画质](historical-evidence-files.md#file-8a935965573d7c6d) |
| ------------------------ | -------------------------------------------------------------: | -----------------------------------------------------------: |
...
每步落点在1,5与1,6交替，代数和撤销深度逐步各+1；起终均为`c.theft.03.t.1.6`、活动独立练习，64格、无暂停、倒数0、无过场、待加载图标0。全部永久字段保持一致。[机器复核](historical-evidence-files.md#file-abb851bbbabcab50)检查了这些关系及原阈值；[低画质实景](historical-evidence-files.md#file-1eb5e0b90e2ed356)在采样结束后拍摄，并已实际查看。

### 8.3 收尾与适用范围

通过正常设置恢复标准画质、退出练习，经区域图返回仓库并逐格走到终端，得到1109代、130/26、默认设置；[正常收尾路线](historical-evidence-files.md#file-66712c4d692616ce)完整保留。服务随后恢复`index-BpL7y5kK.js` production，正常刷新/继续、F触发“已完成，永久结果保留”，再次UI导出仍为1109代，最后返回可游玩画面，见[最终production恢复](historical-evidence-files.md#file-6d13f624d1c68fbf)。
