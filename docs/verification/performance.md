...
核对日期：2026-09-11。**本次必需浏览器已按[工程合同](../spec/architecture.md#3-浏览器与运行预算)收窄为Chrome。本机 M1 Pro / 32 GB 已有最终production正常继续计时，以及同源码观察包的两档64格帧间隔、输入反馈和玩家/镜头完成计量；本组均达到原阈值，30次区域往返的已观测资源计数稳定。原基准设备仍未验证，V05未全量通过。**

Safari两档中位17ms超过16.7ms的失败，及100ms视觉目标下IAB / Safari各六步≤140ms的通过子项，均按原结果保留。Safari / Edge本次不再作为必需浏览器，停止补测不表示Safari帧率已修复。此次未更改任何性能阈值、原基准设备或真实断外网要求。
...
本次继续核对Chrome，原Apple M1 / 8 GB与Windows i5-1135G7 / Iris Xe / 16 GB基准设备尚无实测，机型范围也没有已确认的豁免。M1 Pro / 32 GB的通过子项不替代这两列。
...
Safari两档17ms、旧159ms热身与会话异常保留历史；不再要求独立低档输入统计或环境恢复复测。真实外网断开属于当前仍必需的V06，见[离线资源审计](offline-build-audit.md)，不从本地资源齐全或Chrome性能结果推定通过。
...
本次production还通过了真实旧内存历史恢复的会话保护，详见[浏览器矩阵](browser-matrix.md#7-最终修复版chrome的正常续玩与会话保护)。这些结果不改变原基准设备或真正断外网的未验证边界。
...
没有重跑已通过的30次区域往返，没有修改游戏或测量阈值。原M1/8 GB与Windows指定硬件仍未实测，真实断外网仍未执行；本组不据本机结果宣称这些范围通过，也不将停止Safari验收写成旧失败已修复。
