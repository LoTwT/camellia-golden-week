...
清理后的实现提交为 `878de7429df5313bbb247538e24f6bcb5f3ab57e`，父提交为7fe7d8e；`v0.5.0-r1`绑定该提交。Release以pre-release发布，不设为latest；6个附件均在发布前实际重新下载，SHA-256与本地原件全部相同。之后才按旧OID `083a9be` 为条件替换远端R1分支；main保持原提交。详见[发布回读快照](evidence/r1-cleanup-publication.json)。

从GitHub通过网络执行完整历史的单分支、无标签新克隆，没有复用本地对象；pack为75117126字节（约71.64MiB），跟踪1186文件／122948703字节。冻结安装、682项测试和构建均通过，93文件正式产物与已验收包逐字节一致，工作区干净，证明普通开发不依赖外置证据ZIP。

本快照记录实现提交的实际发布与新克隆结果，随后以仅文档提交补入；最新分支见[PR提交列表](https://github.com/LoTwT/camellia-golden-week/pull/1/commits)。旧本地Git对象、Codex检查点和仓库外备份仍保留，不宣称其占用立即消失。
