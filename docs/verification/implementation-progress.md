...
- 用户已恢复本 Goal：提交溯源明确为 `gpt-6-astra` / `max`，本次必需浏览器改为 Chrome。M1–M4 已验收并正式提交；M5 继续完成 Chrome 剩余预算、离线验证和交付。先前 blocked 记录保留历史含义。
...
M5实现候选已提交 `811da5a1eb6ac175aa380418c2d84cf079469713`；63项中61项按规定层通过。本机Chrome正常全收集链、恢复、会话隔离、两档帧率、输入、加载及移动/镜头预算已通过；原指定M1/8GB与Windows基准机仍未验证，是否改为当前M1 Pro/32GB基准尚待用户答复。

B133浏览器请求隔离尝试未能建立，V06未验证。原生所选页面与验收目标不一致，已停止全局键鼠并仅关闭任务Chrome标签及DevTools；1109真实导出与5174正式服务保留。本轮出现的默认 `*://*` / Block规则删除、Network过滤文字恢复为 `Request conditions` 仍待稳定窗口。已询问用户短暂停止Chrome操作，未获答复前不操作其他页面或继续不可靠点击。硬件范围、离线验证及这段浏览器设置清理之外，独立实现、测试、美术与候选归档已完成；最终推送/PR仍待验收收尾。

最新完整检查 E30 / E31：原生 **411/411**、零失败/取消/跳过/todo，含只读 lint/format/type/content 的生产 build 通过。新包 84 文件 / 1674897B，45 资源记录、78 内容与 2 许可哈希匹配，85 次同源 HTTP 全部 200 且逐字节相同。845629B 静态 ZIP、指纹、启动与剩余条件见[候选交付记录](release-candidate.md)。较早 E29 / B122–B126 的 405 项、旧包和 Safari 结果保留历史含义。

2026-09-11 16:35 后切换固定5174到独立同源码acceptance目录完成Chrome性能；16:44已恢复production，入口与JS逐字节核对通过。生产dist始终未改写，realtime已正常续玩导出收尾：1109代/仓库7,0/26奖励与130物资、默认设置保持。文档维护只做相应格式 / 链接检查，不无故重复已通过的源码测试。
