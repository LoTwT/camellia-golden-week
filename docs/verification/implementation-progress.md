...
M5：63 项映射目前有 61 项按规定层通过。用户本轮取消 Safari / Edge 必需验收；历史 Safari 17ms 与窗口异常不再作为交付门槛。Chrome 当前生产继续 320ms 就绪 / 390ms 内真实移动保存、刷新继续 315ms 已通过。实际双标签与正常历史返回验证了失锁页的原槽、旧内存分别导出并返回会话提示，修复闭环。Chrome同源码观察包移动 / 镜头及两档持续采样现已通过，剩余为原基准设备和真实断外网。已询问是否以本机 M1 Pro / 32GB 为设备基准，尚待答复。
