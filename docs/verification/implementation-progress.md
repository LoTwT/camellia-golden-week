...
### 2026-09-11–12 防火墙原版还原（实现与验证完成）

用户要求“尽可能完全还原原版”，当前范围是防火墙挑战。已核对 S05 画面与 S10 实机录屏：原画面为中央 5×4 格及左右四块 COMBO 电视；原曲为 Red! / Red!! / Red!!!，公开试听实测约 110 BPM，音乐来源与推断边界见新增来源记录。根因是此前复用了通用棋盘与侧栏，且音频只播放固定间隔提示音；单独补白光无法恢复原构图和配乐体验。

当前已接入专用舞台、底部进度/拍点文字、PERFECT/MISS、危险格预告和三首原创替代配乐；尚未确认原录音可随本项目打包，因此不冒充采用原曲。当前内容版本按 M1–M5 升为 2/3/4/5/5、规则版本 2，历史 v1 定义原样保留。27/82 拍、5×4、110 BPM 已进入权威规则；时长、目标、±150ms、奖励和主线规则保持既有约定。旧成绩迁移为历史记录，已领取奖励和进度保留。

实际验证：先加入 5×4 回归，旧实现以 `25 !== 20` 失败；修改后通过。最终 `pnpm run verify` 为 37 文件 / 484 项全过，五分期、七项既有故障回归及三档正常 Chrome 跟拍均通过；198 个实现输入检查前后哈希一致。普通有声和减少效果静音教学各 14 Combo，正式内层 / 深层 / 核心为 47 / 57 / 72，另有正常错拍 5→0 后恢复。三档帧间隔 p95 ≤16.8ms、输入反馈 p95 ≤31.1ms。真实旧 M5 全收集档通过正常导入、刷新和导出，130 物资及九项旧成绩保留。最终包 87 文件 / 6,344,521 字节，ZIP 4,995,583 字节，固定 5174 共 88 次 HTTP 核对通过。完整截图、来源、迁移、红绿回归、性能和剩余差异见[防火墙还原记录](firewall-restoration.md)。发布目标继续为同一实现分支及 PR #1，main 不合并；无剩余必需实现或验证。以下旧交付数字保留历史含义。
...
## 原版视觉节拍核对后的修复（2026-09-11）

用户要求核对原规则后修复拍点表现。原版录屏 `Ua1SdQqcePc` 的21:12起教学明确提示画面周围灯光随拍闪烁；原实现只将节拍状态用于左侧面板，没有挑战画面周边灯光。当前从干净 `aebd63f` 补齐周边白光与减少闪烁模式，并以既有 Chrome 正常新档教学脚本建立先失败后通过的回归。固定地图、计分参数、奖励与存档合同继续沿用当前规格；本轮不把重建参数改称原版测量。旧证据保持原时点含义。

补修已完成：画面周边5px白光读取同一有效时钟，在窗口内亮起、拍点最亮；减少闪烁关闭周边光与侧栏亮暗切换，保留状态文字。四档共用投影，暂停 / 失焦 / 倒数 / 结算熄灭，退出隐藏，覆盖层不接管点击。新字“白”按原固定字体源补齐；未改变规则、奖励、声音或存档。

实际回归先在旧包因缺少周边白光失败，再在新包通过。最终verify为464项原生测试、五分期、七项回归和两种静音教学全部通过，184个所选实现输入哈希不变。两个教学各14次正常按键并自然成功；普通模式实际观察白光后输入，减少闪烁依文字输入。新ZIP848,037字节 / 84文件 / 1,681,345字节，与Chrome构建及dist一致；85次5174回环直连HTTP检查通过。当前服务已加载新静态包，用户页面与存档没有刷新或操作。证据入口见[周边节拍补修](firewall-visual-cue.md#原版核对后的画面周边节拍补修)，同一实现分支和PR交付，最新Git状态以PR head为准。
