...
- Goal 已由 `/goal` 建立且 active；M1–M4 可运行阶段验收完成，进入 M5 视觉、交互、故障与运行验收，不声称全量验收通过。
...
M5：按原参考精修六类场景；完成显示尺寸、键盘、输入时序、故障与会话恢复、性能和离线实测。M1–M4的玩法、全收集与恢复证据已保存。
...
- 63 条验收已建立逐项映射；部分UI故障、键盘边界、视觉、性能与浏览器矩阵仍未验证，实际状态以验收记录为准。
