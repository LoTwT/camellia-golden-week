...
探索时点击远处只会沿已访问的安全格逐步行走。隐藏路线、机关与未领取物资不能靠自动寻路跳过。键盘移动仅在棋盘获得焦点时生效，菜单按钮可用 Tab 操作。防火墙可跟随画面周围白光，也可看左侧拍点条：“现在移动”时按一次方向键，出现“本拍命中”后等待下一拍；鼠标也可点不同格。长按不会自动跟拍。减少闪烁关闭白光和面板亮暗变化，静音或减少动态时仍显示文字提示，见[拍点可读性修复](docs/verification/firewall-visual-cue.md)。
