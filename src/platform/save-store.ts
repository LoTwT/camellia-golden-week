...
export function inspectSaveSlots<T>(
  ...
  if (
    ...
    canonicalJson(slots.a.envelope.payload) !== canonicalJson(slots.b.envelope.payload)
  )
    return {
      ...base,
      status: "conflict",
...
export function createSaveStore<T>(
  ...
  const write = (payload: T, options: SaveWriteOptions<T>): SaveWriteResult<T> => {
    ...
    if (
      options.intent === "restoreBackup" &&
      canonicalJson(validated.value) !== canonicalJson((current.backup ?? current.latest)?.payload)
    )
